...
import dev.risas.dencore.models.configuration.types.WorldAreaWandItem;
...
public class WorldAreaWandButton extends Button {
    ...
    public ItemStack getItemStack(Player player) {
        return new ItemBuilder(Material.STICK)
                .setDisplayName("&6World Team Area Wand")
                .setLore("&7Marcar las posiciones de las áreas de los equipos.")
                .build();
    }
    ...
    public void clicked(Player player, int slot, ClickType clickType, int hotbarButton) {
        ...
        teamController.getTeams().values().forEach(team -> {
            WorldAreaWandItem item = (WorldAreaWandItem) configurationItemController.getConfigurationItemByClass(WorldAreaWandItem.class);
