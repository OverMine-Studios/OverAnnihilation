package dev.risas.dencore.utilities.cuboid;
...
public class Cuboid implements Iterable<Location> {
    ...
    public Location getCenter() {
        return new Location(getWorld(), getLowerX() +
                (double) (getUpperX() - getLowerX()) / 2, getLowerY() +
                (double) (getUpperY() - getLowerY()) / 2, getLowerZ() +
                (double) (getUpperZ() - getLowerZ()) / 2
        );
    }
    ...
    public static Cuboid fromString(String string) {
        ...
        if (parts.length != 7) {
            throw new IllegalArgumentException("Invalid Cuboid string: " + string);
        }
